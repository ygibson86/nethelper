        ...
        {items.map(({ rack, devices: rackSwitches }) => <article className={`rack ${editMode ? 'editable' : ''} ${draggedRackId === rack.id ? 'dragging' : ''}`} key={rack.id} draggable={editMode} onDragStart={(event) => { if (!editMode) return; setDraggedRackId(rack.id); event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', rack.id) }} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = 'move' }} onDrop={(event) => { event.preventDefault(); const sourceId = event.dataTransfer.getData('text/plain'); if (sourceId) { if (rack.group) updateRack(sourceId, { group: rack.group }); moveRackTo(sourceId, rack.id) } setDraggedRackId(null) }} onDragEnd={() => setDraggedRackId(null)}>
          ...
          <div className="rack-frame"><div className="rack-rail left" /><div className="rack-rail right" /><div className="rack-devices">
             {rackSwitches.map((item) => {
               const vendor = manufacturers.find((entry) => entry.id === item.manufacturerId)
               return <div className={`switch-card ${draggedSwitchId === item.id ? 'dragging' : ''} ${switchDropTarget?.id === item.id ? `drop-${switchDropTarget.position}` : ''}`} key={item.id} style={{ '--vendor': vendor?.color ?? '#64748b' } as React.CSSProperties} draggable={editMode} onDragStart={(event) => { event.stopPropagation(); setDraggedSwitchId(item.id); event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', item.id) }} onDragOver={(event) => { if (!editMode) return; event.preventDefault(); event.stopPropagation(); const bounds = event.currentTarget.getBoundingClientRect(); setSwitchDropTarget({ id: item.id, position: event.clientY < bounds.top + bounds.height / 2 ? 'before' : 'after' }) }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setSwitchDropTarget(null) }} onDrop={(event) => { if (!editMode) return; event.preventDefault(); event.stopPropagation(); const sourceId = event.dataTransfer.getData('text/plain'); const position = switchDropTarget?.id === item.id ? switchDropTarget.position : 'before'; if (sourceId) moveSwitchTo(rack.id, sourceId, item.id, position); setDraggedSwitchId(null); setSwitchDropTarget(null) }} onDragEnd={() => { setDraggedSwitchId(null); setSwitchDropTarget(null) }} onClick={() => openDeviceTopology(item)}>

  ...
  return <>
    ...
    {groupedRacks.map(([group, items]) => <section className="rack-group" key={group}>
      ...
      <div className="racks-grid">
        {items.map(({ rack, devices: rackSwitches }) => <article className={`rack ${editMode ? 'editable' : ''} ${draggedRackId === rack.id ? 'dragging' : ''}`} key={rack.id} draggable={editMode} onDragStart={(event) => { if (!editMode) return; setDraggedRackId(rack.id); event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', rack.id) }} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = 'move' }} onDrop={(event) => { event.preventDefault(); const sourceId = event.dataTransfer.getData('text/plain'); if (sourceId) moveRackTo(sourceId, rack.id); setDraggedRackId(null) }} onDragEnd={() => setDraggedRackId(null)}>
          ...
          <div className="rack-frame"><div className="rack-rail left" /><div className="rack-rail right" /><div className="rack-devices">
            {rackSwitches.map((item, index) => {
              const vendor = manufacturers.find((entry) => entry.id === item.manufacturerId)
              return <div className="switch-card" key={item.id} style={{ '--vendor': vendor?.color ?? '#64748b' } as React.CSSProperties} onClick={() => openDeviceTopology(item)}>
    ...
    {editingRack && <Modal title="Редактировать шкаф" onClose={() => setEditingRack(null)}><form className="form-grid" onSubmit={saveRack}><label className="full">Название<input required autoFocus value={rackForm.name} onChange={(event) => setRackForm({ ...rackForm, name: event.target.value })} /></label><label>Расположение<input value={rackForm.location} onChange={(event) => setRackForm({ ...rackForm, location: event.target.value })} /></label><label>Группа<input value={rackForm.group} list="rack-groups" onChange={(event) => setRackForm({ ...rackForm, group: event.target.value })} placeholder="Например, Главный корпус" /><datalist id="rack-groups">{[...new Set(racks.map((rack) => rack.group).filter(Boolean))].map((group) => <option key={group} value={group} />)}</datalist></label><div className="modal-actions full"><button type="button" className="button" onClick={() => setEditingRack(null)}>Отмена</button><button className="button primary">Сохранить</button></div></form></Modal>}

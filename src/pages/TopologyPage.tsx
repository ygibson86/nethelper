...
export function TopologyPage() {
  ...
  const addText = () => {
    if (!active) return
    const textNode: TopologyNode = { id: `text-${Date.now()}`, x: 220, y: 220, label: 'Новый текст', text: 'Новый текст', type: 'text', color: '#000000', fontSize: 18, handles: { top: 0, bottom: 0, left: 0, right: 0 } }
  ...
  return <div className="topology-page">
    ...
    <div className="topology-workspace"><aside className="topology-list"><div className="section-title"><span>Все схемы</span><button className="icon-button" onClick={createTopology}><Plus size={17} /></button></div><div className="topology-search"><Search size={14} /><input value={topologyQuery} onChange={(event) => setTopologyQuery(event.target.value)} placeholder="Поиск схем..." /></div><div className="topology-items">{filteredTopologies.map((item) => <button key={item.id} className={`topology-item ${item.id === active?.id ? 'active' : ''}`} onClick={() => navigate(`/topology/${item.id}`)}><span><strong>{item.name}</strong><small>{item.nodes.length} узлов · {item.links.length} связей</small></span></button>)}</div>{active && <div className="topology-manage"><button onClick={() => { const name = prompt('Новое название', active.name); if (name?.trim()) updateTopology(active.id, { name: name.trim() }) }}><Edit3 size={15} /> Переименовать</button><button className="danger-text" onClick={() => { if (confirm(`Удалить схему «${active.name}»?`)) { deleteTopology(active.id); navigate('/topology') } }}><Trash2 size={15} /> Удалить</button></div>}</aside>

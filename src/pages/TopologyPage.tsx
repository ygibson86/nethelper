...
import { BatteryCharging, Box, Cable, Camera, Cloud, Copy, Database, Edit3, Eye, Group, Monitor, Network, PanelsTopLeft, Phone, Plus, Printer, Radio, Router, Save, Search, Server, Shield, Trash2, Type, X, type LucideIcon } from 'lucide-react'
...
import { useSaveState } from '../components/saveState'

interface DeviceNodeData extends Record<string, unknown> { label: string; name?: string; ip?: string; text?: string; type: DeviceType; color: string; manufacturerId?: string; handles: HandleLayout; switchId?: string; highlighted?: boolean; width?: number; height?: number; fontSize?: number; groupLabelPosition?: 'top' | 'center'; resizeGroup?: (width: number, height: number) => void }
type DeviceFlowNode = Node<DeviceNodeData, 'device' | 'group' | 'text'>
interface LinkEdgeData extends Record<string, unknown> { sourcePort: string; targetPort: string; cableType: CableType; label?: string; showLabel?: boolean }
type LinkFlowEdge = Edge<LinkEdgeData, 'networkLink'>

type LinkForm = Omit<TopologyLink, 'id'> & { id?: string }
type DeletionSnapshot = { topologyId: string; nodes: TopologyNode[]; links: TopologyLink[]; message: string }
...
const deviceLabels: Record<DeviceType, string> = { switch: 'Коммутатор', router: 'Маршрутизатор', pc: 'Компьютер', server: 'Сервер', firewall: 'Межсетевой экран', 'access-point': 'Точка доступа', printer: 'Принтер', phone: 'IP-телефон', camera: 'Камера', cloud: 'Облако / Интернет', ups: 'ИБП', nas: 'Сетевое хранилище', 'patch-panel': 'Патч-панель', text: 'Текст', group: 'Группа / шкаф' }
const deviceIcons: Record<DeviceType, LucideIcon> = { switch: Network, router: Router, pc: Monitor, server: Server, firewall: Shield, 'access-point': Radio, printer: Printer, phone: Phone, camera: Camera, cloud: Cloud, ups: BatteryCharging, nas: Database, 'patch-panel': PanelsTopLeft, text: Type, group: Group }
...
function NetworkLinkEdge({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, markerEnd, style, data, selected }: EdgeProps) {
  ...
  return <>
    ...
    <EdgeLabelRenderer>
       ...
       {linkData?.showLabel !== false && linkData?.label && <span className="edge-detail-label" style={{ transform: `translate(-50%, -50%) translate(${((sourceX + targetX) / 2)}px,${((sourceY + targetY) / 2)}px)` }}>{linkData?.label}</span>}
    </EdgeLabelRenderer>
  </>
}
...
function TextNode({ data, selected }: NodeProps<DeviceFlowNode>) {
  return <div className={`flow-text-node ${selected ? 'selected' : ''}`} style={{ color: data.color, fontSize: `${data.fontSize ?? 18}px` }}>{data.label}</div>
}
const nodeTypes = { device: DeviceNode, group: GroupNode, text: TextNode }
...
export function TopologyPage() {
  ...
  const { topologies, switches, manufacturers, settings, addTopology, updateTopology, deleteTopology } = useNetHelper()
  ...
  const [undoHistory, setUndoHistory] = useState<DeletionSnapshot[]>([])
  ...
  useEffect(() => {
    setNodes((current) => (active?.nodes ?? []).map((node) => {
      ...
      return {
        id: node.id,
         type: node.type === 'group' ? 'group' : node.type === 'text' ? 'text' : 'device',
  ...
  useEffect(() => {
    setEdges((current) => (active?.links ?? []).map((link) => {
      ...
      return { id: link.id, source: link.source, target: link.target, sourceHandle: link.sourceHandle, targetHandle: link.targetHandle, type: 'networkLink', selected: previous?.selected, zIndex: previous?.zIndex ?? 0, data: { sourcePort: link.sourcePort, targetPort: link.targetPort, cableType: link.cableType, label: link.label, showLabel: link.showLabel }, style: { strokeWidth: highlightedPort && (link.sourcePort === highlightedPort || link.targetPort === highlightedPort) ? 4 : 2, stroke: cableColors[link.cableType] } }
    }))
  ...
  const pushDeletion = useCallback((snapshot: DeletionSnapshot) => {
    setUndoHistory((history) => [...history, snapshot].slice(-20))
  ...
  const onNodesDelete = useCallback((deleted: DeviceFlowNode[]) => {
    ...
    const deletedNodes = active.nodes.filter((node) => ids.has(node.id))
    const deletedLinks = active.links.filter((link) => ids.has(link.source) || ids.has(link.target))
    pushDeletion({ topologyId: active.id, nodes: deletedNodes, links: deletedLinks, message: `Удалено узлов: ${deletedNodes.length}` })
  ...
  }, [active, editMode, updateTopology, pushDeletion])
  const onEdgesDelete = useCallback((deleted: LinkFlowEdge[]) => {
    if (!active || !editMode) return
    const ids = new Set(deleted.map((edge) => edge.id))
    const deletedLinks = active.links.filter((link) => ids.has(link.id))
    setUndoHistory((current) => {
      const last = current[current.length - 1]
      if (last?.topologyId !== active.id) return [...current, { topologyId: active.id, nodes: [], links: deletedLinks, message: `Удалено связей: ${deletedLinks.length}` }].slice(-20)
      const savedLinkIds = new Set(last.links.map((link) => link.id))
      return [...current.slice(0, -1), { ...last, links: [...last.links, ...deletedLinks.filter((link) => !savedLinkIds.has(link.id))] }]
    })
    setUndoNoticeVisible(true)
  ...
  const addText = () => {
    if (!active) return
    const textNode: TopologyNode = { id: `text-${Date.now()}`, x: 220, y: 220, label: 'Новый текст', text: 'Новый текст', type: 'text', color: settings.theme === 'dark' ? '#e7ecf4' : '#18202c', fontSize: 18, handles: { top: 0, bottom: 0, left: 0, right: 0 } }
    updateTopology(active.id, { nodes: [...active.nodes, textNode] }); setEditingNodeId(textNode.id)
  }
  ...
  const restoreDeletion = useCallback(() => {
    const snapshot = undoHistory.at(-1)
    if (!snapshot) return
    const topology = topologies.find((item) => item.id === snapshot.topologyId)
    if (!topology) return
    const nodeIds = new Set(topology.nodes.map((node) => node.id))
    const linkIds = new Set(topology.links.map((link) => link.id))
    updateTopology(topology.id, {
      nodes: [...topology.nodes, ...snapshot.nodes.filter((node) => !nodeIds.has(node.id))],
      links: [...topology.links, ...snapshot.links.filter((link) => !linkIds.has(link.id))],
    })
    setUndoHistory((history) => history.slice(0, -1))
    setUndoNoticeVisible(false)
  }, [topologies, undoHistory, updateTopology])
  useEffect(() => {
    if (!undoNoticeVisible) return
    const timer = window.setTimeout(() => setUndoNoticeVisible(false), 8000)
    return () => window.clearTimeout(timer)
  }, [undoNoticeVisible])
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      const target = event.target
      const isFormField = target instanceof HTMLInputElement || target instanceof HTMLTextAreaElement || target instanceof HTMLSelectElement || target instanceof HTMLButtonElement || (target instanceof HTMLElement && (target.isContentEditable || Boolean(target.closest('input, textarea, select, button, [contenteditable="true"]'))))
      if (!editMode || isFormField) return
      ...
      if (event.key === 'Delete') {
        ...
        if (active && (nodeIds.size || edgeIds.size)) {
          event.preventDefault()
          const deletedNodes = active.nodes.filter((node) => nodeIds.has(node.id))
          const deletedLinks = active.links.filter((link) => nodeIds.has(link.source) || nodeIds.has(link.target) || edgeIds.has(link.id))
          pushDeletion({ topologyId: active.id, nodes: deletedNodes, links: deletedLinks, message: nodeIds.size ? `Удалено узлов: ${nodeIds.size}` : `Удалено связей: ${edgeIds.size}` })
  ...
  const deleteNodeWithUndo = (nodeId: string) => {
    if (!active) return
    const deletedNodes = active.nodes.filter((node) => node.id === nodeId)
    const deletedLinks = active.links.filter((link) => link.source === nodeId || link.target === nodeId)
    pushDeletion({ topologyId: active.id, nodes: deletedNodes, links: deletedLinks, message: `Удалено узлов: ${deletedNodes.length}` })
    updateTopology(active.id, { nodes: active.nodes.filter((node) => node.id !== nodeId), links: active.links.filter((link) => link.source !== nodeId && link.target !== nodeId) })
  }
  const deleteLinkWithUndo = (linkId: string | undefined) => {
    if (!active || !linkId) return
    const deletedLinks = active.links.filter((link) => link.id === linkId)
    pushDeletion({ topologyId: active.id, nodes: [], links: deletedLinks, message: `Удалено связей: ${deletedLinks.length}` })
    updateTopology(active.id, { links: active.links.filter((link) => link.id !== linkId) })
  }
  ...
  const viewingDevice = switches.find((device) => device.id === viewingNode?.switchId)
  const openSshForNode = (node: TopologyNode) => {
    const path = viewingDevice?.accessMethods?.includes('ssh') ? `/terminal/${encodeURIComponent(viewingDevice.id)}` : node.ip?.trim() ? `/terminal?host=${encodeURIComponent(node.ip.trim())}` : ''
    if (path) window.open(`${window.location.origin}${window.location.pathname}#${path}`, '_blank', 'noopener')
  }
  const createTopology = () => { const name = prompt('Название новой схемы'); if (name?.trim()) navigate(`/topology/${addTopology(name.trim())}`) }
  const saveStatusLabel = saveState.status === 'saving' ? 'Сохранение…' : saveState.status === 'dirty' ? 'Не сохранено' : saveState.status === 'error' ? 'Ошибка сохранения' : saveState.status === 'conflict' ? 'Конфликт изменений' : 'Сохранено'
  const clearTrace = () => {
  ...
  return <div className="topology-page">
    <header className="page-header compact"><div><p className="eyebrow">Редактор сети</p><h1>Схемы</h1>{highlightedPort && <p className="trace-hint"><Cable size={14} /> Трассировка от порта {highlightedPort}</p>}</div><div className="toolbar"><div className="mode-switch"><button aria-pressed={!editMode} className={!editMode ? 'active' : ''} onClick={() => setEditMode(false)}><Eye size={16} /> Просмотр</button><button aria-pressed={editMode} className={editMode ? 'active' : ''} onClick={() => setEditMode(true)}><Edit3 size={16} /> Редактирование</button></div>{editMode && <><button className="button" onClick={copySelected} disabled={!nodes.some((node) => node.selected)}><Copy size={16} /> Копировать</button><button className="button" onClick={pasteNodes} disabled={!copiedNodes.length}><Box size={16} /> Вставить</button><button className="button" onClick={() => setLinkForm({ ...emptyLink })}><Cable size={16} /> Связь</button><button className="button" onClick={addGroup}><Group size={16} /> Группа</button><button className="button" onClick={addText}><Type size={16} /> Текст</button><button className="button primary" onClick={addNode}><Plus size={17} /> Узел</button></>}</div></header>
    <div className="topology-workspace"><aside className="topology-list"><div className="section-title"><span>Все схемы</span><button className="icon-button" onClick={createTopology} aria-label="Создать схему"><Plus size={17} /></button></div><div className="topology-search"><Search size={14} /><input aria-label="Поиск схем" value={topologyQuery} onChange={(event) => setTopologyQuery(event.target.value)} placeholder="Поиск схем..." /></div><div className="topology-items">{filteredTopologies.map((item) => <button key={item.id} className={`topology-item ${item.id === active?.id ? 'active' : ''}`} onClick={() => navigate(`/topology/${item.id}`)}><span><strong>{item.name}</strong><small>{item.nodes.length} узлов · {item.links.length} связей</small></span></button>)}</div>{topologyQuery && !filteredTopologies.length && <p className="list-empty">Схемы не найдены</p>}{active && <div className="topology-manage"><button onClick={() => { const name = prompt('Новое название', active.name); if (name?.trim()) updateTopology(active.id, { name: name.trim() }) }}><Edit3 size={15} /> Переименовать</button><button className="danger-text" onClick={() => { if (confirm(`Удалить схему «${active.name}»?`)) { deleteTopology(active.id); navigate('/topology') } }}><Trash2 size={15} /> Удалить</button></div>}</aside>
      <section className="canvas-panel">{active ? <><div className="canvas-title"><div><strong>{active.name}</strong><span>{active.description || 'Дважды нажмите узел или связь для настройки'}</span></div><span className={`autosave save-${saveState.status}`} role="status" title={saveState.message}><Save size={14} /> {saveStatusLabel}</span></div><div className="flow-wrap"><ReactFlow<DeviceFlowNode, LinkFlowEdge> nodes={nodes} edges={edges} nodeTypes={nodeTypes} edgeTypes={edgeTypes} onInit={setFlowInstance} connectionMode={ConnectionMode.Loose} snapToGrid snapGrid={[20, 20]} multiSelectionKeyCode={['Control', 'Meta']} selectionKeyCode={null} selectionOnDrag={editMode} selectionMode={SelectionMode.Partial} panOnDrag={editMode ? [1, 2] : true} nodesDraggable={editMode} nodesConnectable={editMode} elementsSelectable onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onNodeDragStop={onNodeDragStop} onNodesDelete={onNodesDelete} onEdgesDelete={onEdgesDelete} deleteKeyCode={null} onConnect={onConnect} onNodeDoubleClick={(_, node) => { if (!editMode) { clearTrace(); setEditMode(true); setViewingNodeId(null) } setEditingNodeId(node.id) }} onEdgeDoubleClick={(_, edge) => { const link = active.links.find((item) => item.id === edge.id); if (link) { if (!editMode) { clearTrace(); setEditMode(true); setViewingNodeId(null) } setLinkForm({ ...link }) } }} onNodeClick={(event, node) => { if (!editMode) selectViewNode(node.id, event.ctrlKey || event.metaKey) }} onPaneClick={() => { if (!editMode) clearViewSelection() }} zoomOnDoubleClick={false} colorMode={settings.theme}><Background gap={20} size={1} /><Controls /><MiniMap pannable zoomable nodeColor={(node) => String(node.data?.color ?? '#64748b')} /></ReactFlow></div></> : <div className="empty-state"><Network size={42} /><h2>Нет схем</h2><button className="button primary" onClick={createTopology}>Создать схему</button></div>}</section>
    </div>
    {currentNode && currentNode.type !== 'text' && active && <Modal title="Настройка узла" onClose={() => setEditingNodeId(null)}><form className="form-grid" onSubmit={(event) => event.preventDefault()}>{currentNode.type === 'group' ? <label className="full">Наименование<input value={currentNode.label} onChange={(event) => updateTopology(active.id, { nodes: active.nodes.map((node) => node.id === currentNode.id ? { ...node, label: event.target.value } : node) })} /></label> : <><label className="full">Наименование<input value={currentNode.name ?? ''} onChange={(event) => updateTopology(active.id, { nodes: active.nodes.map((node) => node.id === currentNode.id ? { ...node, name: event.target.value } : node) })} /></label><label className="full">Имя хоста<input value={currentNode.label} onChange={(event) => updateTopology(active.id, { nodes: active.nodes.map((node) => node.id === currentNode.id ? { ...node, label: event.target.value } : node) })} /></label></>}{currentNode.type !== 'group' && <label>Тип<select value={currentNode.type} onChange={(event) => updateTopology(active.id, { nodes: active.nodes.map((node) => node.id === currentNode.id ? { ...node, type: event.target.value as DeviceType } : node) })}>{Object.entries(deviceLabels).filter(([value]) => value !== 'group').map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>}<label>Цвет<input type="color" value={currentNode.color} onChange={(event) => updateTopology(active.id, { nodes: active.nodes.map((node) => node.id === currentNode.id ? { ...node, color: event.target.value } : node) })} /></label>{currentNode.type !== 'group' && <label className="full">Связанное устройство<select value={currentNode.switchId ?? ''} onChange={(event) => { const device = switches.find((item) => item.id === event.target.value); updateTopology(active.id, { nodes: active.nodes.map((node) => node.id === currentNode.id ? { ...node, switchId: device?.id, ip: device?.ip || node.ip, label: device?.hostname || node.label, manufacturerId: device?.manufacturerId || node.manufacturerId, color: manufacturers.find((item) => item.id === device?.manufacturerId)?.color ?? node.color } : node) }) }}><option value="">Не связано</option>{switches.map((device) => <option key={device.id} value={device.id}>{device.hostname}{device.ip ? ` · ${device.ip}` : ''}</option>)}</select></label>}{currentNode.type !== 'group' && <label>Производитель<select value={currentNode.manufacturerId ?? ''} onChange={(event) => { const vendor = manufacturers.find((item) => item.id === event.target.value); updateTopology(active.id, { nodes: active.nodes.map((node) => node.id === currentNode.id ? { ...node, manufacturerId: vendor?.id, color: vendor?.color ?? node.color } : node) }) }}><option value="">Не выбран</option>{manufacturers.filter((item) => item.deviceTypes.includes(currentNode.type)).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}{currentNode.type !== 'group' && <label className="full">IP-адрес<input value={currentNode.ip ?? ''} onChange={(event) => updateTopology(active.id, { nodes: active.nodes.map((node) => node.id === currentNode.id ? { ...node, ip: event.target.value } : node) })} /></label>}{currentNode.type === 'group' && <><label>Положение названия<select value={currentNode.groupLabelPosition ?? 'top'} onChange={(event) => updateTopology(active.id, { nodes: active.nodes.map((node) => node.id === currentNode.id ? { ...node, groupLabelPosition: event.target.value as 'top' | 'center' } : node) })}><option value="top">Сверху по центру</option><option value="center">По центру</option></select></label><p className="group-resize-help full">Размер группы изменяется маркерами по краям прямо на холсте.</p></>}{currentNode.type !== 'group' && <div className="handle-settings full"><strong>Точки подключения</strong><span>Укажите количество точек на каждой стороне узла</span><div>{(['top', 'bottom', 'left', 'right'] as HandleSide[]).map((side) => <label key={side}>{side === 'top' ? 'Сверху' : side === 'bottom' ? 'Снизу' : side === 'left' ? 'Слева' : 'Справа'}<input type="number" min="0" max="8" value={(currentNode.handles ?? defaultHandles)[side]} onChange={(event) => updateTopology(active.id, { nodes: active.nodes.map((node) => node.id === currentNode.id ? { ...node, handles: { ...(node.handles ?? defaultHandles), [side]: Math.max(0, Math.min(8, Number(event.target.value))) } } : node) })} /></label>)}</div></div>}<div className="modal-actions full"><button type="button" className="button danger-button" onClick={() => { deleteNodeWithUndo(currentNode.id); setEditingNodeId(null) }}><Trash2 size={16} /> Удалить</button><button className="button primary" type="button" onClick={() => setEditingNodeId(null)}>Готово</button></div></form></Modal>}
    {currentNode?.type === 'text' && active && <Modal title="Настройка текста" onClose={() => setEditingNodeId(null)}><form className="form-grid" onSubmit={(event) => event.preventDefault()}><label className="full">Текст<textarea autoFocus value={currentNode.label} onChange={(event) => updateTopology(active.id, { nodes: active.nodes.map((node) => node.id === currentNode.id ? { ...node, label: event.target.value, text: event.target.value } : node) })} /></label><label>Цвет<input type="color" value={currentNode.color} onChange={(event) => updateTopology(active.id, { nodes: active.nodes.map((node) => node.id === currentNode.id ? { ...node, color: event.target.value } : node) })} /></label><label>Размер шрифта<input type="number" min={12} max={96} value={currentNode.fontSize ?? 18} onChange={(event) => updateTopology(active.id, { nodes: active.nodes.map((node) => node.id === currentNode.id ? { ...node, fontSize: Number(event.target.value) } : node) })} /></label><div className="modal-actions full"><button type="button" className="button" onClick={() => setEditingNodeId(null)}>Закрыть</button></div></form></Modal>}
     {viewingNode && <aside className="node-inspector"><button className="icon-button node-inspector-close" onClick={clearViewSelection} aria-label="Закрыть сведения об узле"><X size={16} /></button><div className="node-view-summary"><span className="flow-icon" style={{ '--node-color': viewingNode.color } as React.CSSProperties}><NodeTypeIcon type={viewingNode.type} size={22} /></span><div><strong>{viewingNode.name || viewingNode.label}</strong>{viewingNode.name && <span>{viewingNode.label}</span>}<span>{viewingNode.ip || 'IP-адрес не указан'}</span></div></div><small>{deviceLabels[viewingNode.type]}</small>{viewingNode.ip?.trim() || viewingDevice?.accessMethods?.includes('ssh') ? <button className="button" onClick={() => openSshForNode(viewingNode)}>Подключиться по SSH</button> : <button className="button" disabled>SSH недоступен — укажите IP или свяжите устройство</button>}<button className="button" disabled>Открыть веб-интерфейс — в разработке</button></aside>}
    {undoNoticeVisible && undoHistory.length > 0 && <div className="undo-toast" role="status"><span>{undoHistory.at(-1)?.message}</span><button className="button" onClick={restoreDeletion}>Отменить (Ctrl+Z)</button><button className="icon-button" onClick={() => setUndoNoticeVisible(false)} aria-label="Закрыть уведомление"><X size={15} /></button></div>}
    {linkForm && active && <Modal title={linkForm.id ? 'Редактировать связь' : 'Новая связь'} onClose={() => setLinkForm(null)}><form className="form-grid" onSubmit={saveLink}><label>От устройства<select required value={linkForm.source} onChange={(event) => setLinkForm({ ...linkForm, source: event.target.value })}><option value="">Выберите</option>{active.nodes.map((node) => <option key={node.id} value={node.id}>{node.label}</option>)}</select></label><label>Порт источника<input required value={linkForm.sourcePort} onChange={(event) => setLinkForm({ ...linkForm, sourcePort: event.target.value })} placeholder="1/0/1" /></label><label>К устройству<select required value={linkForm.target} onChange={(event) => setLinkForm({ ...linkForm, target: event.target.value })}><option value="">Выберите</option>{active.nodes.map((node) => <option key={node.id} value={node.id}>{node.label}</option>)}</select></label><label>Порт назначения<input required value={linkForm.targetPort} onChange={(event) => setLinkForm({ ...linkForm, targetPort: event.target.value })} placeholder="1/0/48" /></label><label>Тип кабеля<select value={linkForm.cableType} onChange={(event) => setLinkForm({ ...linkForm, cableType: event.target.value as CableType })}>{Object.entries(cableLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="full">Комментарий<input value={linkForm.label ?? ''} onChange={(event) => setLinkForm({ ...linkForm, label: event.target.value })} placeholder="Магистраль, патч-панель..." /></label><label className="checkbox full"><input type="checkbox" checked={linkForm.showLabel !== false} onChange={(event) => setLinkForm({ ...linkForm, showLabel: event.target.checked })} /> Показывать на схеме</label><div className="modal-actions full">{linkForm.id && <button type="button" className="button danger-button" onClick={() => { deleteLinkWithUndo(linkForm.id); setLinkForm(null) }}><Trash2 size={16} /> Удалить</button>}<button type="button" className="button" onClick={() => setLinkForm(null)}>Отмена</button><button className="button primary">Сохранить</button></div></form></Modal>}
  </div>
}

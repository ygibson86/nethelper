import { Cable, Edit3, ExternalLink, Plus, Save, Trash2 } from 'lucide-react'
...
export function CorePage() {
  const { corePanels, switches, topologies, addCorePanel, updateCorePanel, deleteCorePanel } = useNetHelper()
  const [editMode, setEditMode] = useState(false)
  const [editingPort, setEditingPort] = useState<{ panelId: string; portId: string } | null>(null)
  const [portDraft, setPortDraft] = useState<CorePort | null>(null)
  ...
  const availableSwitches = useMemo(() => switches.filter((item) => item.isCore && !corePanels.some((panel) => panel.switchId === item.id)), [corePanels, switches])

  const openPortEditor = (panelId: string, port: CorePort) => {
    setEditingPort({ panelId, portId: port.id })
    setPortDraft({ ...port })
  }
  ...
  const savePort = () => {
    if (!activePanel || !activePort || !portDraft) return
    updateCorePanel(activePanel.id, { ports: activePanel.ports.map((port) => port.id === activePort.id ? portDraft : port) })
    closePortEditor()
  }
  ...
  const addPanel = () => {
    if (!availableSwitches.length) return alert('Все отмеченные Core-коммутаторы уже добавлены. Новое устройство можно отметить как Core на странице «Шкафы».')
    setSelectedSwitchId(availableSwitches[0].id)
    setAddingPanel(true)
  }
  const confirmAddPanel = () => {
    if (!selectedSwitchId) return
    addCorePanel(selectedSwitchId, 28)
    setAddingPanel(false)
    setSelectedSwitchId('')
  }

  return <>
    <header className="page-header"><div><p className="eyebrow">Мониторинг портов</p><h1>Core-коммутаторы</h1><p className="page-subtitle">{corePanels.length} устройств · {totalPorts} портов</p></div><div className="toolbar"><button className="button" onClick={addPanel}><Plus size={17} /> Добавить панель</button><button className={`button ${editMode ? 'primary' : ''}`} aria-pressed={editMode} onClick={() => setEditMode(!editMode)}>{editMode ? <Save size={17} /> : <Edit3 size={17} />}{editMode ? 'Готово' : 'Редактировать'}</button></div></header>
    {corePanels.length ? <div className="core-stack">{corePanels.map((panel) => {
      ...
      return <article className="core-device" key={panel.id}>
        <section className="core-summary"><div className="core-ident"><span className="core-icon"><Cable size={25} /></span><div><strong>{device?.hostname ?? 'Неизвестное устройство'}</strong><span>{device?.model || 'Модель не указана'} · {device?.ip || 'IP не указан'}</span></div></div><div className="port-stats"><span><i className="status active" /> {stats.active} активных</span><span><i className="status inactive" /> {stats.inactive} неактивных</span><span><i className="status disabled" /> {stats.disabled} отключено</span></div>{editMode && <div className="core-actions"><button className="button" onClick={() => setLayoutPanelId(panel.id)}><Cable size={16} /> {panel.ports.length} портов · {panel.rows?.length || 1} ряд.</button><button className="icon-button danger" aria-label={`Удалить панель ${device?.hostname ?? ''}`} onClick={() => confirm(`Удалить панель «${device?.hostname ?? 'Неизвестное устройство'}»?`) && deleteCorePanel(panel.id)}><Trash2 size={17} /></button></div>}</section>
        <section className="front-panel"><div className="panel-screws"><i /><span>{device?.hostname} · {device?.model || 'FRONT VIEW'}</span><i /></div><div className="core-port-rows">{rows.map((row) => <div className="core-port-row" key={row.id}><span className="core-row-label">{row.label}</span><div className="core-ports-grid">{row.portIds.map((portId) => portsById.get(portId)).filter((port): port is CorePort => Boolean(port)).map((port) => <button key={port.id} className={`port ${port.status} ${port.identifier.includes('/1/') ? 'uplink-port' : ''} ${port.identifier.endsWith('/1/1') ? 'uplink-start' : ''}`} title={`${port.identifier} · ${statusLabels[port.status]}${port.label ? ` · ${port.label}` : ''}`} onClick={() => editMode ? openPortEditor(panel.id, port) : port.ip && followIp(port.ip, port.identifier)}><span className="port-led" /><strong>{port.identifier}</strong><span className="port-socket"><i /><i /><i /><i /></span><small>{port.ip || '—'}</small>{port.ip && !editMode && <ExternalLink size={11} />}</button>)}</div></div>)}</div></section>
      </article>
    ...
    {addingPanel && <Modal title="Добавить Core-панель" onClose={() => setAddingPanel(false)}><form className="form-grid" onSubmit={(event) => { event.preventDefault(); confirmAddPanel() }}><label className="full">Core-коммутатор<select autoFocus required value={selectedSwitchId} onChange={(event) => setSelectedSwitchId(event.target.value)}>{availableSwitches.map((item) => <option key={item.id} value={item.id}>{item.hostname}{item.ip ? ` · ${item.ip}` : ''}</option>)}</select></label><div className="modal-actions full"><button type="button" className="button" onClick={() => setAddingPanel(false)}>Отмена</button><button className="button primary">Добавить</button></div></form></Modal>}
    ...
    {portDraft && activePanel && activePort && <Modal title={`Порт ${activePort.identifier}`} onClose={closePortEditor}><form className="form-grid" onSubmit={(event) => { event.preventDefault(); savePort() }}><label>Идентификатор<input value={portDraft.identifier} onChange={(event) => setPortDraft({ ...portDraft, identifier: event.target.value })} /></label><label>Состояние<select value={portDraft.status} onChange={(event) => setPortDraft({ ...portDraft, status: event.target.value as PortStatus })}><option value="active">Активен</option><option value="inactive">Неактивен</option><option value="disabled">Отключён</option></select></label><label className="full">IP подключённого устройства<input value={portDraft.ip} onChange={(event) => setPortDraft({ ...portDraft, ip: event.target.value })} placeholder="10.20.0.10" /></label><label className="full">Подпись<input value={portDraft.label} onChange={(event) => setPortDraft({ ...portDraft, label: event.target.value })} placeholder="Назначение порта" /></label><div className="modal-actions full"><button type="button" className="button" onClick={closePortEditor}>Отмена</button><button className="button primary">Сохранить</button></div></form></Modal>}
  </>
}

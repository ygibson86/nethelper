...
- Выделенный текст автоматически копируется в буфер; `Ctrl+C` копирует выделение, а без выделения отправляется в SSH-сессию; правый клик вставляет текст из буфера обмена.
